...
class EEGDataset(Dataset):
    def __init__(self, config, mode):
        ...
        if config['data']['blur_type']['target'].rsplit('.', 1)[-1] == 'UniformBlur':
            data_dir = os.path.join(self.data_dir, '../Image_feature',
                                    f"{config['data']['blur_type']['target'].rsplit('.', 1)[-1]}_{config['data']['blur_type']['params']['blur_kernel_size']}")
        else:
            data_dir = os.path.join(self.data_dir, '../Image_feature',
        ...
        if len(name_parts) >= 4 and name_parts[1] == 'ubp':
            image_feature_name = '_'.join([name_parts[0], name_parts[1], name_parts[3]])
        else:
            image_feature_name = self.name

        features_filename = os.path.join(data_dir, f"Fovea_blur_{image_feature_name}_{mode}.pt")
        features_filename2 = os.path.join(data_dir, f"Gaussian_noise_{image_feature_name}_{mode}.pt")
        features_filename4 = os.path.join(data_dir, f"LowRes_{image_feature_name}_{mode}.pt")
        features_filename3 = os.path.join(data_dir, f"Mosaic_{image_feature_name}_{mode}.pt")

        pretrain_map = {
            'RN50': {
                'pretrained': 'openai', 'resize': (224, 224)},  # 1024
        ...
        if os.path.exists(features_filename2):
            # if False:
            saved_features2 = torch.load(features_filename2, weights_only=False)
            self.img_features2 = saved_features2['img_features']
            self.text_features2 = saved_features2['text_features']

        if os.path.exists(features_filename3):
            # if False:
            saved_features3 = torch.load(features_filename3, weights_only=False)
            self.img_features3 = saved_features3['img_features']
            self.text_features3 = saved_features3['text_features']

        if os.path.exists(features_filename4):
            # if False:
            saved_features4 = torch.load(features_filename4, weights_only=False)
            self.img_features4 = saved_features4['img_features']
            self.text_features4 = saved_features4['text_features']
            ...
            if self.config['data']['uncertainty_aware']:
                ...
                self.blur_transform2 = instantiate_from_config(config['data']['blur_type2'])
                self.img_features2 = self.ImageEncoder2(self.loaded_data[0]['img'])
                self.img_features3 = self.ImageEncoder3(self.loaded_data[0]['img'])
                self.img_features4 = self.ImageEncoder4(self.loaded_data[0]['img'])
            ...
            self.text_features2 = self.Textencoder(self.loaded_data[0]['text'])
            self.text_features3 = self.Textencoder(self.loaded_data[0]['text'])
            self.text_features4 = self.Textencoder(self.loaded_data[0]['text'])
            torch.save({
            ...
            torch.save({
                'text_features': self.text_features2,
                'img_features': self.img_features2,
            }, features_filename2)

            torch.save({
                'text_features': self.text_features3,
                'img_features': self.img_features3,
            }, features_filename3)

            torch.save({
                'text_features': self.text_features4,
                'img_features': self.img_features4,
            }, features_filename4)
    ...
    @torch.no_grad()
    def ImageEncoder4(self, images, blur_transform=None):
        if blur_transform == None:
            blur_transform = self.blur_transform2
        self.vlmodel.eval()
        ...
        for i in tqdm(range(0, len(set_images), batch_size)):
            ...
            ele = [self.process_transform(
                blur_transform(
                    Image.open(os.path.join(self.data_dir, '../Image_set_LowRes', img)).convert("RGB"))) for
                img in batch_images]
    ...
    @torch.no_grad()
    def ImageEncoder3(self, images, blur_transform=None):
        if blur_transform == None:
            blur_transform = self.blur_transform2
        self.vlmodel.eval()
        ...
        for i in tqdm(range(0, len(set_images), batch_size)):
            ...
            ele = [self.process_transform(
                blur_transform(
                    Image.open(os.path.join(self.data_dir, '../Image_set_Mosaic', img)).convert("RGB"))) for
                img in batch_images]
    ...
    @torch.no_grad()
    def ImageEncoder2(self, images, blur_transform=None):
        if blur_transform == None:
            blur_transform = self.blur_transform2
        self.vlmodel.eval()
        ...
        for i in tqdm(range(0, len(set_images), batch_size)):
            ...
            ele = [self.process_transform(
                blur_transform(
                    Image.open(os.path.join(self.data_dir, '../Image_set_Gaussian', img)).convert("RGB"))) for
                img in batch_images]
    ...
    def ImageEncoder(self, images, blur_transform=None):
        ...
        for i in tqdm(range(0, len(set_images), batch_size)):
            ...
            device = next(self.vlmodel.parameters()).device
            ele = [self.process_transform(
                blur_transform(Image.open(os.path.join(self.data_dir, '../Image_set_Resize', img)).convert("RGB"))) for
                   img in batch_images]
    ...
    def __getitem__(self, index):
        ...
        if self.config['data']['uncertainty_aware']:
            ...
            img_features2 = self.img_features2[img_path]
            img_features3 = self.img_features3[img_path]
            img_features4 = self.img_features4[img_path]
        else:
        ...
        text_features2 = self.text_features2[self.loaded_data[subject]['text'][trial_index]]
        text_features3 = self.text_features3[self.loaded_data[subject]['text'][trial_index]]
        text_features4 = self.text_features4[self.loaded_data[subject]['text'][trial_index]]
        ...
        sample = {
            ...
            'img_features2': img_features2,
            'img_features3': img_features3,
            'img_features4': img_features4,
            'text': text,
            'text_features': text_features,
            'text_features2': text_features2,
            'text_features3': text_features3,
            'text_features4': text_features4,
            'session': session,
            'subject': subject,
